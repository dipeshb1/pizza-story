      ...
      <Form method="POST">
        ...
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <div className="grow">
            ...
            {formErrors?.phone && (
              <p className="mt-0.5 rounded-sm bg-red-100 px-2 py-1 text-sm text-red-600">
                {formErrors.phone}
              </p>
            )}
          </div>
        ...
        <div className="relative mb-4 flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          {!position.latitude && !position.longitude && (
            <span className="z-999 absolute right-0 top-[-3px] sm:right-[3px] sm:top-[3px]">
              <Button
                type="small"
                onClick={(e) => {
                  e.preventDefault();
        ...
        <div>
          ...
          <Button type="primary" disabled={isSubmitting || isLoadingAddress}>
            {isSubmitting
              ? "Placing order..."
              : `Order now ${formatCurrency(totalAmount)}`}
          </Button>
        </div>
      </Form>
    </div>
